...
{  
  ...
  PIDController pid_vel_xyz_controller(20.0, 0.9, 0.0);
  ...
  while (true)
  {
    ...
    if (i % 10 == 0)
    {
      ...
      auto pid_vel_xyz_controller_io_output = pid_vel_xyz_controller.computeControlSignal(bracelet_link_coord_twist.vel, 
      ...
      {
        if (cod[j] == 1){
          control_accelerations(j) = pid_vel_xyz_controller_io_output(j);
        }
      }
    }
  ...
  while (true)
  {
    ...
    if (i % 10 == 0)
    {
      ...
      auto pid_vel_y_controller_io_output = pid_vel_y_controller.computeControlSignal(bracelet_link_coord_twist.vel, 
          control_twist.vel, 0.1);
  ...
  int move_arm_vel_xyz_status = move_arm_vel_xyz();
  if (move_arm_vel_xyz_status != 1)
  {
    std::cout << "Motion move_arm_vel_xyz failed" << std::endl;
    return 0;
  }
  
  int move_arm_vel_xyz_1d_status = move_arm_vel_xyz_1d();
  if (move_arm_vel_xyz_1d_status != 1)
  {
    std::cout << "Motion move_arm_vel_xyz_1d failed" << std::endl;
    return 0;
  }
