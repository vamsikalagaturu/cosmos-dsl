...
public:
     ...
     * @param alpha_unit_forces The output Jacobian matrix of unit constraint forces for end effector.
     ...
     */
    static std::tuple<std::array<double, 3>, std::array<double, 3>> computeFK(KDL::Chain *robot_chain,
                                                                               KDL::JntArray& q);

    /**
     * @brief Updates the beta_energy vector with the given control acceleration enegy input and compensates for gravity.
    ...
    */
    static void updateBetaEnergy(KDL::JntArray& beta_energy, const std::array<double, 6>& control_ae = {0, 0, 0, 0, 0, 0});
     ...
     */
    static void updateQandQd(KDL::JntArray& q, KDL::JntArray& qd, const KDL::JntArray& qdd, double dt);
