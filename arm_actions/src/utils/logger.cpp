...
std::string Logger::string_format(const std::string& format, Args... args) {
    ...
    for (size_t i = 0; i < format.size(); ++i) {
        if (format[i] == '%') {
            ...
            std::visit([&](auto&& arg) {
                format_helper(ss, format.substr(i, 1), arg);
...
std::string Logger::currentTimestamp() {
    ...
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()) % (60 * 1000);

    std::ostringstream timestamp;
    timestamp << std::put_time(&now_tm, "%H:%M:") << std::setw(5) << std::setfill('0') << now_ms.count();
    if (show_date_) {
