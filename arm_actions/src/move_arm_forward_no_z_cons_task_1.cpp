...
int main()
{
  ...
  auto move_arm_forward_no_z_cons = [&]()
  {
    ...
    target_link_ef_coord_frame.p = {0.1, 0.0, 0.0};
    ...
    PIDController pid_bracelet_link_target_link_ef_xy_dist_controller(30.0, 0.9, 0.0, 0.1);
    ...
    // set the solver parameters
    KDL::JntArray ff_tau(7);          // input feedforward torques
    KDL::JntArray constraint_tau(7);  // output constraint torques
    ...
    if (_plot)
    {
      ...
      plotter->plotXYZ(positions, target_link_ef_coord_frame.p, "poss");
    }
